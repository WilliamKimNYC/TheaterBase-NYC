...
def update_urls():
    ...
    for theater in app.config['theaters'].values():
        if theater.get('banner'):
            theater['banner'] = theater['banner'].replace('https://wnskim.github.io/TheaterBase-NYC/', '/')
        if theater.get('image'):
            theater['image'] = theater['image'].replace('https://wnskim.github.io/TheaterBase-NYC/', '/')
